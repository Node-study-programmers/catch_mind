        ...
        <div className="w-full mt-20 h-full overflow-scroll">
          {sortedUserData?.map((user, index) => (
            ...
            >
              ...
              <img className="rounded-full w-14 h-14" src={`${import.meta.env.VITE_IMG_URL}${user.profileImage}`} />
              <div className="overflow-hidden text-ellipsis whitespace-nowrap relative">{user.nickname}</div>
              <div className="text-2xl text-teal-500" style={{ fontFamily: "Rubik Mono One" }}>
                {user.score}점
              </div>
            </div>
          ))}
